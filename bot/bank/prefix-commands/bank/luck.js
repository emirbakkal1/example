...
module.exports = {
  data: {
    name: "luck",
    description: "Receive a random money deposit based on luck.",
  },
  /**
   *
  ...
  async execute(message, args) {
    ...
    // Define the range for the random deposit
    const MIN_DEPOSIT = 3000; // Minimum deposit amount
    const MAX_DEPOSIT = 30000; // Maximum deposit amount
    ...
    if (settings?.canvas) {
      ...
      ctx.fillStyle = '#ffffff';
      ctx.fillText("You recieved a random deposit!", textJobX, textJobY + 10); // Draw the text
    ...
    // Send a confirmation message
      ...
      .setDescription(`Congratulations! You received a random deposit of **${randomDeposit.toLocaleString('en-US')}** coins.\nYour current balance is **${(randomDeposit + user.balance).toLocaleString('en-US')}**`)
