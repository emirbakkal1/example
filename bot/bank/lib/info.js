...
const getTokenId = async (botId) => {
  const tokenIdRecord = await prisma.token.findFirst({
    where: {
      botId,
    },
    select: {
  ...
  if (!tokenIdRecord) {
    throw new Error(`No token found for botId: ${botId}`);
  }

  return tokenIdRecord.id;
};
...
const getEmbed = async (botId) => {
  ...
  if (!setting) {
    throw new Error(`No settings found for botId: ${botId}`);
  }
